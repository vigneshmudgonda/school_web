...
const AddSchool = () => {
  ...
  const [fileError, setFileError] = useState(""); // <-- for invalid file format
  ...
  const allowedFormats = ["image/jpeg", "image/jpg", "image/png"]; // Allowed MIME types
  ...
  const handleImageChange = (e) => {
    ...
    if (selected) {
      if (!allowedFormats.includes(selected.type)) {
        setFile(null);
        setPreview(null);
        setFileError("Invalid file format! Only JPG, JPEG, PNG are allowed.");
        return;
      }
      setFile(selected);
    ...
    <div style={{ maxWidth: "500px", margin: "20px auto", padding: "20px", border: "1px solid #ddd", borderRadius: "8px" }}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data">
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "10px" }}>
          ...
          />
          <select {...register("state", { required: "State is required" })} style={{ flex: 1, padding: "8px" }}>
