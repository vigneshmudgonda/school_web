...
const AddSchool = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      alert(
        err.response?.data?.error || "Failed to add school. Check backend logs."
      );
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data">
        ...
        {(errors.city || errors.state) && (
          <p style={{ color: "red" }}>City and State are required</p>
        )}
        ...
        <div style={{ marginBottom: "10px" }}>
         <input
  type="file"
  name="image"                     // <-- MUST match Multer field name
  accept="image/*"
...
/><input
  type="file"
  name="image" // must match upload.single("image") in backend
  accept="image/*"
  {...register("image", { required: "Image is required" })}
  onChange={handleImageChange}
/>
