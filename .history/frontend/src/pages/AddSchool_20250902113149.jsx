import React, { useState } from "react";
import { useForm } from "react-hook-form";
import axios from "axios";

const AddSchool = () => {
  const [preview, setPreview] = useState(null);
  const [loading, setLoading] = useState(false);

  const { register, handleSubmit, formState: { errors }, reset } = useForm();
  const API_URL = process.env.REACT_APP_API_URL || "http://localhost:5000";

  const onSubmit = async (data) => {
    if (!data.image[0]) {
      alert("Please select an image!");
      return;
    }

    try {
      setLoading(true);

      const formData = new FormData();
      formData.append("name", data.name);
      formData.append("address", data.address);
      formData.append("city", data.city);
      formData.append("state", data.state);
      formData.append("contact", data.contact);
      formData.append("email_id", data.email_id);
      formData.append("image", data.image[0]); // Must match backend field name

      const res = await axios.post(`${API_URL}/schools`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      alert(res.data.message || "School added successfully");
      reset();
      setPreview(null);
    } catch (err) {
      console.error("Add school error:", err.response || err.message);
      alert(err.response?.data?.error || "Failed to add school");
    } finally {
      setLoading(false);
    }
  };

  const handleImageChange = (e) => {
    const file = e.target.files[0];
    if (file) setPreview(URL.createObjectURL(file));
    else setPreview(null);
  };

  return (
    <div style={{ maxWidth: "500px", margin: "20px auto", padding: "20px", border: "1px solid #ddd", borderRadius: "8px" }}>
      <h2 style={{ textAlign: "center" }}>Add New School</h2>

      <form onSubmit={handleSubmit(onSubmit)}>
        {/* Name */}
        <input type="text" placeholder="School Name" {...register("name", { required: "Name is required" })} />
        {errors.name && <p style={{ color: "red" }}>{errors.name.message}</p>}

        {/* Address */}
        <input type="text" placeholder="Address" {...register("address", { required: "Address is required" })} />
        {errors.address && <p style={{ color: "red" }}>{errors.address.message}</p>}

        {/* City & State */}
        <div style={{ display: "flex", gap: "10px" }}>
          <input type="text" placeholder="City" {...register("city", { required: "City is required" })} />
          <select {...register("state", { required: "State is required" })}>
            <option value="">Select State</option>
            <option value="Andhra Pradesh">Andhra Pradesh</option>
            <option value="Karnataka">Karnataka</option>
            <option value="Tamil Nadu">Tamil Nadu</option>
            <option value="Maharashtra">Maharashtra</option>
            <option value="Delhi">Delhi</option>
          </select>
        </div>
        {(errors.city || errors.state) && <p style={{ color: "red" }}>City and State are required</p>}

        {/* Contact */}
        <input type="tel" placeholder="Contact" {...register("contact", { required: "Contact is required", pattern: { value: /^[0-9]{10}$/, message: "10 digits required" } })} />
        {errors.contact && <p style={{ color: "red" }}>{errors.contact.message}</p>}

        {/* Email */}
        <input type="email" placeholder="Email" {...register("email_id", { required: "Email is required", pattern: { value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, message: "Invalid email" } })} />
        {errors.email_id && <p style={{ color: "red" }}>{errors.email_id.message}</p>}

        {/* Image */}
        <input type="file" accept="image/*" {...register("image", { required: "Image is required" })} onChange={handleImageChange} />
        {errors.image && <p style={{ color: "red" }}>{errors.image.message}</p>}

        {preview && <img src={preview} alt="Preview" style={{ width: "150px", margin: "10px 0", borderRadius: "8px" }} />}

        <button type="submit" disabled={loading}>{loading ? "Submitting..." : "Add School"}</button>
      </form>
    </div>
  );
};

export default AddSchool;
